...
db.prepare('INSERT OR IGNORE INTO professions(name) VALUES(?)').run(n)
);
}
...
res.json(
db.prepare('SELECT * FROM professions ORDER BY name').all()
)
);
app.get('/api/trainings', auth, (req, res) =>
res.json(
db.prepare(`
SELECT tr.*, p.name profession
FROM trainings tr
JOIN professions p ON p.id = tr.profession_id
...
const r = db.prepare(`
INSERT INTO trainings(profession_id,name,levels,pass_mark)
VALUES(?,?,?,?)
`).run(
...
(trainee_id,level,score,grade,status,date)
...
(certificate_no,trainee_id,final_score,final_grade,status)
VALUES(?,?,?,?, 'PENDING')
`).run(
no,
t.id,
...
`).run(
s,
req.body.checked_by || req.user.username,
s,
req.body.remarks || '',
c.id
);
...
active: db.prepare(
"SELECT COUNT(*) c FROM trainees WHERE status='Active'"
).get().c,
completed: db.prepare(
"SELECT COUNT(*) c FROM trainees WHERE status='Completed'"
).get().c,
pending: db.prepare(
"SELECT COUNT(*) c FROM certificates WHERE status='PENDING'"
).get().c,
approved: db.prepare(
"SELECT COUNT(*) c FROM certificates WHERE status='APPROVED'"
).get().c
})
);
